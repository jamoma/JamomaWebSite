...
You can find a cutting edge package below.
...
<notextile>
  <?php
    ...
    date_default_timezone_set('Europe/London');
    // second argument as 1 means "descendent"
    $versions = scandir( './nightly-builds', 1 );
    ...
    foreach ( $versions as $version) {
      ...
      if( $version == '.' ) continue;
      if( $version == '..' ) continue;
      if( $version == 'Readme' ) continue;

      $date = date( 'jS \of F Y', filemtime( "nightly-builds/$version" ) );

      ?>

        Download build: <a href=<?php echo "'nightly-builds/$version'" ?>><?php echo $version ?></a> - <?php echo $date ?>
        <br />
