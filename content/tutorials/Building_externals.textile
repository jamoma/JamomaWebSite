...
bq(note). For users incomfortable with the Terminal, just type the command @cd@ followed by a space and drop the Tools folder from the Finder to the Terminal window.
...
bq(note). There are also scripts to build only one Jamoma sub-project at a time (e.g. only DSP or modular).
