...
Whereas Max's "loadbang":http://cycling74.com/docs/max5/refpages/max-ref/loadbang.html  object can be used to know when a patch is properly loaded, it is of no use when trying to get when a Jamoma module is properly initialized. Indeed, for each module, a number of operations, such as initialization of "jcom.parameters":components/jcom.parameter/, loading of preset, etc. has to be done by "jcom.hub":/tutorials/components/jcom.hub/.  Thus Jamoma has a specific object somewhat similar to "loadbang":http://cycling74.com/docs/max5/refpages/max-ref/loadbang.html that comes to help.

"jcom.init":/tutorials/components/jcom.init/ works like a "loadbang":http://cycling74.com/docs/max5/refpages/max-ref/loadbang.html but is local to a module. That is, it connects to "jcom.hub":/tutorials/components/jcom.hub/ found in the same abstraction and outputs a @(max)bang@ when the module is properly initialized. Note that "jcom.init":/tutorials/components/jcom.init/ can be placed in any sub-patcher as long as it does not contain another "jcom.hub":/tutorials/components/jcom.hub/. 
...
In this example, we built a similar situation using three modules from Jamoma Modular core distribution ("jmod.qmetro":/tutorials/modules/jmod.qmetro that we will use as a clock, "jmod.10harmonics~":/tutorials/modules/jmod.10harmonics~/ that we will use as mini synthesizer and "jmod.output~":/tutorials/modules/jmod.output~ so you can enjoy the wonderful 5 notes-based arpeggio composed for this example...). Similarly to the example aforementioned, we want to make sure that our synth is properly initialized before starting to play our sequence.  "jcom.initialized":/tutorials/components/jcom.initialized/ can then be used to send a @(max)bang@ when the module defined as its argument is properly initialized. From then, the @(max)bang@ message can trigger the message box that it is connected to and send to "jmod.qmetro /myClock":/tutorials/modules/jmod.qmetro the following couple of message to set a speed and to turn it on.

bc. /time 200
/toggle 1

bq(note). Remember that there is currently no way to define an initialization order for each module. While the patch we used works for our demonstration purpose, it may be trickier in a complex patch involving a lot of modules. For example, you would probably want to make sure @(max)myClock@ is also properly initialized before receiving any message. We will let this as an exercise for the reader.

"Fine. But how does this really works?" Glad you asked.

"jcom.initialized":/tutorials/components/jcom.initialized/ is a Max patch-based component (_abstraction_) that wraps a special feature of "jcom.receive":/tutorials/components/jcom.receive/. 

!images/jcom.initialization_inside.png!

When provided @(max)'notifications'@ as argument, "jcom.receive":/tutorials/components/jcom.receive can be used to notify of various operations done in Jamoma framework, i.e module creation, module deletion and more importantly here: module initialization. Thus, for each module in a patch, "jcom.receive notifications":/tutorials/components/jcom.receive will send the message @(max)module.initialized@ followed by the class of the module and its instance name. In the previously used example, "jcom.receive":/tutorials/components/jcom.receive would send the following messages: 
...
Thus "jcom.initialized":/tutorials/components/jcom.initialized/ is a handy component that filters out notifications sent by "jcom.receive notifications":/tutorials/components/jcom.receive and sends a @(max)bang@ when matching initialization notification for a given named module defined as argument.

h3. Download

"Here you can download the patch used for this tutorial (right-click and save).":attachments/how-to-get-jcom-hub-s-initialization-notification.maxpat

h3(seealso). See also

* "jcom.init":/tutorials/components/jcom.init/
* "jmod.initialization":/tutorials/modules/jmod.initialization
* "jcom.receive":/tutorials/components/jcom.receive
