...
We'll now build the communication between this algorithm and our User Interface objects, through custom Jamoma _components_ named "jcom.parameter":../../components/jcom.parameter/  and "jcom.message":../../components/jcom.message/ .
...
Now we're ready for some magic. Try turning the toggle on and off, and change the time in the interface. If everything is working as it's supposed to, any change you make will cause the message box further down in the patch (connected to the 2nd outlet of "jcom.pass":../../components/jcom.pass/ ) to reflect the parameter change.

h3. More about jcom.parameter

We now need to take a look at the arguments and attributes for "jcom.parameter":../../components/jcom.parameter/ .
...
Before we move on, it's worth noting that "jcom.parameter":../../components/jcom.parameter/  is quite powerful. In addition to keeping track of the current state of the parameter, it can be used for ramping to new values, clipping the range and filtering out repetitions.

h3. Returned message

We want our module to return a message everytime the "qmetro":http://cycling74.com/docs/max5/refpages/max-ref/qmetro.html fires. In order to be OSC compatible we do not want to return a 'bang'. Instead we will return the message '/trigger bang'. In order to do so we will use the component "jcom.return":../../components/jcom.return/. "jcom.return":../../components/jcom.return/ takes more or less the same arguments and attributes as "jcom.parameter":../../components/jcom.parameter/, and we will use the following settings:
...
Why do we do it this way instead of simply prepending @/trigger@? The answer is that "jcom.return":../../components/jcom.return/ helps us in the documentation of the module, as well as with various ways of communication and mapping between modules that will be documented in other tutorials.

h3. The interface

Next we will create the interface. If you look in the upper left corner of the patch, there's a kind of panel displaying @/editing_this_module@. This is "jcom.ui":../../components/jcom.ui/, one of the core externals of Jamoma, which will be the background of your user interface.
...
But first, switch from patching mode to presentation mode. Then move the comments and UI objects to where you like on jcom.ui. If you like, resize "jcom.ui":../../components/jcom.ui/ so that it fits to the size of your interface. You should get something like this :
...
First we want to change the description attribute for "jcom.hub":../../components/jcom.hub/ accordingly to what our module does.

If you want you can connect the left outlet of "jcom.hub":../../components/jcom.hub/ and the left outlet of the algorithm to a button to check that the patch is working. This is what it ought to look like:
...
By clicking the %(samp)Open reference page% in "jcom.ui":../../components/jcom.ui/ 's topleft menu, the documentation page should open and look something like "this":../../modules/jmod.qmetro:
...
* "jcom.parameter":../../components/jcom.parameter/  is dealing with one parameter.
* If you need to communicate messages to the module that you do not want to treat as parameters, you can use "jcom.message":../../components/jcom.message/ .
* "jcom.return":../../components/jcom.return/ is used to get some variables out of the module
* "jcom.hub":../../components/jcom.hub/ is the brain of the Jamoma system, a hub communicating back and forth with all other Jamoma components in the module.
* "jcom.ui":../../components/jcom.ui/ is providing the backdrop for the GUI part. It also embeds a number of common features that you want in all or some of the modules. This will be detailed further in the tutorials on how to build audio and video modules.
...
* Design the user interface. If you need the interface to be a different size, you can change it by resizing "jcom.ui":../../components/jcom.ui/ accordingly.
* Create "jcom.parameter":../../components/jcom.parameter/, "jcom.message":../../components/jcom.message/ and "jcom.return":../../components/jcom.return objects as required, and set arguments and attributes.
