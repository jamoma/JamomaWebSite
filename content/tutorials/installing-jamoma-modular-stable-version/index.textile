...
Installer can be downloaded from "Jamoma web site":/. Please visit jamoma.org %(seq)1% and follow the "download link":/download/ %(seq)2%.
...
bq(note). For some reasons (i.e. installing an older version of Jamoma Modular), if you ever needed to uninstall Jamoma Modular, you will find in the disk image a shell script to automate the process of removing all installed Jamoma files.
...
Max is now properly set up for Jamoma. Have fun exploring the various provided "modules":/tutorials/modules/ and examples !
