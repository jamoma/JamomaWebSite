...
We will start of with much the same patch that we ended up with in the "previous tutorial":/tutorials/introduction-to-jamoma-modules, containing an audio input and output module so that we can get sound into and out of the patch. Instead of just continue using the same patch, we'll start out with a fresh blank patcher window, as this gives us an excuse to introduce a few useful shortcuts.
...
* With the patch in edit mode, we can type <code>SHIFT + I</code> to create an input module. The module will show up at the position the mouse is pointing at when we type the shortcut.
* In the same way we can type <code>SHIFT + O</code> to create an output module.

bq(note). The "Shortcuts_in_Jamoma":/tutorials/shortcuts-in-jamoma lists all available key combinations in Jamoma.
...
We want to load the "jmod.echo~":/tutorials/modules/jmod.echo~ module, and give it the name _echo~_, so we will type in bpacther @@args myecho~ @name jmod.echo~@.
...
If you want to apply the Jamoma MIDI range elsewhere in your patches and modules, the external "jcom.dataspace":/tutorials/components/jcom.dataspace can be used for mappings between linear, decibel and midi representations of gain levels. Set %(samp)@dataspace% to @gain@, and choose the @input and @output attributes appropriate for what you want to do. Please refer to the help file for "jcom.dataspace":/tutorials/components/jcom.dataspace/ for further details.
...
* Shortcuts in Jamoma.
** Type <code>SHIFT + I</code> to create an input module.
** Type <code>SHIFT + O</code> to create an output module.
** Type <code>SHIFT + B</code> to create any module:
...
For the next tutorial we are going to take a look at "how to communicate with the module":/tutorials/sending-messages-to-modules/.

h3. Download

"Here you can download the patch used for this tutorial (right-click and save).":attachments/sound-fx-modules.maxpat
