...
As stated above, Jamoma modules are patches or abstractions, so they can be instantiated by typing the name of the module into a new object box. We will start of with jmod.input~, and give it the argument %(samp)/myinput~%.
...
* Open the inspector and set the patcher file to be "jmod.input~":/tutorials/modules/jmod.input~/ and provide %(samp)/myinput~% as argument to the "bpatcher":http://cycling74.com/docs/max5/refpages/max-ref/bpatcher.html:
...
bq(note). In Jamoma modules the leftmost inlet and outlet is reserved for control messages to and from the module. All remaining inlets and outlets are used for passing signals in and out of the module.
...
* The output level is controlled by the horizontal slider.This slider is one of Jamoma's custom UI objects: "jcom.textslider":/tutorials/components/jcom.textslider/. See the "Shortcuts in Jamoma":/tutorials/shortcuts-in-jamoma/ to learn about the features of "jcom.textslider":/tutorials/components/jcom.textslider/.
* The two meters indicate levels on a logarithmic (dB) scale. This is done using one of the custom externals developed for Jamoma: "jcom.meter~":/tutorials/components/jcom.meter~/. This level indicator can be useful as a GUI object on its own, and we suggest that you check out the help file for it.
...
* "jmod.input~":/tutorials/modules/jmod.input~/ and "jmod.output~":/tutorials/modules/jmod.output~/ can be provide generic audio input and output to and from the patch.

h3. Download

"Here you can download the patch used for this tutorial (right-click and save).":attachments/introduction-to-jamoma-modules.maxpat


h4. What's Next?

Now that we have audio input and output working, it is tempting to introduce some "sound effect processing":/tutorials/sound-fx-modules/ between the input and output module.
