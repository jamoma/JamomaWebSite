...
A filter or a limiter can come in handy in all sorts of audio processing models. Previosuly this caused us to implement limiters and their parameters several times over for different modules, in a way that could not be said to be DRY (Don't Repeat Yourself).

In Jamoma 0.6 it is possible to embed models in models. The output~ model is one example that makes extensive use of it. It offers saturation and limiter master effects as well as stereo balance, and in 0.6 these functions are impemented by embedding the preexisting saturation~, limiter~ and balance~ models:

p=. <%= fancyImage @item, "images/embedded-models.png", "images/embedded-models.png", "Models embedded in the output~ model", "embedded-models" %>
...
p=. <%= fancyImage @item, "images/output-namespace.png", "images/output-namespace-thumb.png", "output~.model namespace", "embedded-models" %>